...
- **`backend-laravel/tests/Feature/SecurityPoliciesTest.php`, `ModuleManagerTest.php`, `UserEscalationGuardTest.php`, `PublicEndpointsClosedTest.php`** — 20 tests / 146 aserciones, corren contra sqlite en memoria (`RefreshDatabase`), nada toca datos reales. Cubren exactamente los 4 fixes de hoy: Policies con ownership, `ModuleManager`, el bloqueo de auto-escalación a `super_admin`, y los 4 endpoints que eran públicos. Correrlos con `php artisan test tests/Feature/NombreDelTest.php` (uno por uno) — **NO** con `php artisan test` a secas.
