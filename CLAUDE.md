...
- **Rediseño completo Roles↔Permisos**: el panel "Roles" de Filament sigue sin controlar de verdad lo que cada rol puede hacer (las Policies usan `hasAnyRole()` hardcodeado, no los permisos Spatie que se editan ahí — mismo motivo por el que Hugo vio "Administrador" sin nada tildado en Roles pero con acceso a Usuarios igual). Sigue pendiente — no se tocó al converger con `core/v1` (ver más abajo), porque el sistema de módulos de esa rama es un eje distinto (qué funcionalidades existen en la instalación) del sistema de permisos (qué puede hacer cada usuario). Queda como próximo paso real.
...
**Hallazgo de paso, más de fondo:** `App\Models\Notification` (el modelo detrás de ese Resource) **no coincide con el esquema real de la tabla `notifications`** — la tabla es la estándar de Laravel (`notifiable_type`/`notifiable_id`/`data` JSON), no las columnas `user_id`/`title`/`message` que el modelo declara. Sin ninguna otra referencia en el codebase. Es decir: **es código muerto que nunca pudo funcionar** — la campanita de notificaciones real usa el sistema nativo de Filament sobre la tabla estándar. No se borró nada sin que Hugo lo confirme. **Candidato para la próxima sesión:** decidir si se borra `NotificationResource` + `App\Models\Notification` + su migración/vistas asociadas (probablemente lo correcto, dado que es inalcanzable) o si en algún momento se quiere arreglar el esquema para usarlo de verdad.
