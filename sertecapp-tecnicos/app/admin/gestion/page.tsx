  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      <header className="bg-red-600 text-white px-4 py-4 flex items-center gap-3 shadow-lg">
        ...
        <button onClick={abrirNuevo} className="ml-auto bg-white dark:bg-gray-800 text-red-600 dark:text-red-400 font-semibold text-sm px-4 py-2 rounded-xl hover:bg-red-50 dark:hover:bg-red-900 transition-colors">
          + Nuevo
        </button>
      </header>
      ...
      <main className="p-4 max-w-lg mx-auto mt-4">
        {loading ? (
          ...
          <div className="space-y-3">
            {users.map(u => (
              <div key={u.id} className={`bg-white dark:bg-gray-800 rounded-2xl shadow-sm border p-4 ${!u.is_active ? 'opacity-50 border-gray-200 dark:border-gray-700' : 'border-gray-100 dark:border-gray-700'}`}>
        ...
        {error && !showModal && <div className="mt-4 bg-red-50 dark:bg-red-900 border border-red-200 dark:border-red-700 text-red-700 dark:text-red-400 px-4 py-3 rounded-xl text-sm">{error}</div>}
      </main>

      {/* Modal crear/editar */}
      {showModal && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4">
          <form onSubmit={(e) => { e.preventDefault(); guardar(); }} className="bg-white dark:bg-gray-800 w-full sm:max-w-md rounded-t-3xl sm:rounded-3xl shadow-2xl max-h-[90vh] overflow-y-auto">
            <div className="px-6 py-4 border-b border-gray-100 dark:border-gray-700 flex items-center justify-between sticky top-0 bg-white dark:bg-gray-800 rounded-t-3xl">
              <h2 className="font-bold text-gray-800 dark:text-gray-100 text-lg">{editUser ? 'Editar Usuario' : 'Nuevo Usuario'}</h2>
              <button type="button" onClick={() => setShowModal(false)} className="text-gray-400 dark:text-gray-500 hover:text-gray-600 dark:hover:text-gray-300 text-2xl leading-none">×</button>
            </div>
