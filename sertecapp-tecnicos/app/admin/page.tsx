...
const selectClass = "w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-xl text-sm focus:ring-2 focus:ring-red-500 bg-white dark:bg-gray-800";
      ...
      <div className="min-h-screen bg-gray-900 dark:bg-gray-950">
        <header className="bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 sticky top-0 z-10">
          <div className="max-w-4xl mx-auto px-4 py-3 flex items-center gap-3">
            ...
            <p className="font-semibold text-gray-900 dark:text-white text-sm">Vista Técnico — {user?.name}</p>
          </div>
        </header>
        <div className="max-w-4xl mx-auto px-4 py-4">
          {loadingTecnico ? (
            ...
            <div className="bg-white dark:bg-gray-800 rounded-2xl p-8 text-center text-gray-400 dark:text-gray-500 mt-4">
            ...
            <>
              {pendientes.length > 0 && (
                <div className="mb-4">
                  <p className="text-xs font-semibold text-gray-400 dark:text-gray-500 uppercase tracking-wider mb-2 px-1">Pendientes ({pendientes.length})</p>
                  <div className="space-y-3">
                    {pendientes.map(o => (
                      <div key={o.id} className={`bg-white dark:bg-gray-800 rounded-xl shadow-sm border-l-4 ${prioColor[o.priority] || 'border-l-gray-300'} p-4`}>
                        <div className="flex items-start justify-between mb-1">
                          <p className="font-semibold text-gray-900 dark:text-white text-sm">{o.clientName || o.customer?.business_name || 'Sin cliente'}</p>
                          <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${statusColor[o.status] || 'bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300'}`}>{normalizeStatus(o.status)}</span>
                        </div>
                        <p className="text-sm text-gray-600 dark:text-gray-300 mb-2">{o.problem || o.title}</p>
                        {o.address && <p className="text-xs text-gray-400 dark:text-gray-500">📍 {o.address}</p>}
              ...
              {completadas.length > 0 && (
                <div>
                  <p className="text-xs font-semibold text-gray-400 dark:text-gray-500 uppercase tracking-wider mb-2 px-1">Completadas ({completadas.length})</p>
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      ...
      <main className="p-4 max-w-4xl mx-auto">
        <div className="grid grid-cols-2 gap-3 mb-4 mt-4">
          {[{ label: 'Órdenes', value: stats.ordenes, icon: '📋' }, { label: 'Clientes', value: stats.clientes, icon: '👥' },
            ...
            <div key={s.label} className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl p-4 flex items-center gap-3 shadow-sm">
              ...
              <div><p className="text-2xl font-bold text-gray-800 dark:text-gray-100">{loading ? '...' : s.value}</p><p className="text-xs text-gray-500 dark:text-gray-400">{s.label}</p></div>
            </div>
          ))}
        </div>
        ...
        <div className="grid grid-cols-2 gap-3 mb-3">
          <button onClick={() => verVistasTecnico(user?.id)} className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl p-4 flex items-center gap-3 shadow-sm hover:shadow-md transition-all text-left w-full">
        ...
        {tecnicos.length > 0 && (
          <div className="mb-4">
            <p className="text-xs font-semibold text-gray-400 dark:text-gray-500 uppercase tracking-wider mb-2 px-1">Ver como técnico</p>
            <div className="flex gap-2 flex-wrap">
              {tecnicos.map(t => (
                <button key={t.id} onClick={() => verVistasTecnico(t.id)} className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl px-3 py-2 text-sm font-medium text-gray-700 dark:text-gray-300 hover:bg-red-50 dark:hover:bg-red-900 hover:border-red-300 hover:text-red-700 dark:hover:text-red-400 transition-all shadow-sm">
                  👷 {t.name}
                </button>
              ))}
            </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden">
          <div className="px-4 py-3 border-b border-gray-100 dark:border-gray-700 flex items-center justify-between">
            <h2 className="font-semibold text-gray-800 dark:text-gray-100">Órdenes de Trabajo</h2>
          ...
          {loading ? <div className="p-8 text-center text-gray-400 dark:text-gray-500">Cargando...</div>
            : ordenes.length === 0 ? <div className="p-8 text-center text-gray-400 dark:text-gray-500">No hay órdenes</div>
            : <div className="divide-y divide-gray-50">
              {ordenes.map((o) => (
                <div key={o.id} className="px-4 py-3 flex items-center justify-between hover:bg-gray-50 dark:hover:bg-gray-900 cursor-pointer" onClick={() => router.push('/admin/orden?id=' + String(o.id))}>
                  <div>
                    <p className="font-medium text-gray-800 dark:text-gray-100 text-sm">#{o.id} — {o.customer?.business_name || o.customer?.full_name || 'Sin cliente'}</p>
                    <p className="text-xs text-gray-500 dark:text-gray-400">{o.title}</p>
                    {o.assigned_tech && <p className="text-xs text-blue-500">👷 {o.assigned_tech.name}</p>}
                  </div>
                  <span className={`text-xs px-2 py-1 rounded-full font-medium ${statusColor[o.status] || 'bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300'}`}>{normalizeStatus(o.status)}</span>
                </div>
              ))}
            </div>}
        </div>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4">
          <div className="bg-white dark:bg-gray-800 w-full sm:max-w-lg rounded-t-3xl sm:rounded-3xl shadow-2xl max-h-[90vh] overflow-y-auto">
            <div className="px-6 py-4 border-b border-gray-100 dark:border-gray-700 flex items-center justify-between sticky top-0 bg-white dark:bg-gray-800 rounded-t-3xl">
              <h2 className="font-bold text-gray-800 dark:text-gray-100 text-lg">Nueva Orden</h2>
            ...
            <div className="p-6 space-y-4">
              ...
              <div>
                ...
                <select value={form.equipment_id} onChange={e => setForm(f => ({ ...f, equipment_id: e.target.value }))} disabled={!form.customer_id} style={{color:'#111827'}} className={`${selectClass} disabled:bg-gray-50 dark:disabled:bg-gray-900 disabled:text-gray-400 dark:disabled:text-gray-500`}>
              ...
              <div>
                ...
                <input value={form.title} onChange={e => setForm(f => ({ ...f, title: e.target.value }))} placeholder="Ej: Reparación cinta caminadora" style={{color:'#111827'}} className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-xl text-sm focus:ring-2 focus:ring-red-500 bg-white dark:bg-gray-800" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Descripción</label>
                <textarea value={form.description} onChange={e => setForm(f => ({ ...f, description: e.target.value }))} rows={3} placeholder="Detalles del problema..." style={{color:'#111827'}} className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-xl text-sm focus:ring-2 focus:ring-red-500 resize-none bg-white dark:bg-gray-800" />
              </div>
              ...
              <div>
                ...
                <input type="date" value={form.scheduled_date} onChange={e => setForm(f => ({ ...f, scheduled_date: e.target.value }))} style={{color:'#111827'}} className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-xl text-sm focus:ring-2 focus:ring-red-500 bg-white dark:bg-gray-800" />
              </div>
              ...
              {formError && <div className="bg-red-50 dark:bg-red-900 border border-red-200 dark:border-red-700 text-red-700 dark:text-red-400 px-4 py-3 rounded-xl text-sm">{formError}</div>}
