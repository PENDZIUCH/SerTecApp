    ...
    >
      ...
      {suggestedParts.length > 0 && (
        <div className="mb-3 p-3 bg-gray-50 rounded-md">
          <p className="text-xs font-medium text-gray-700 mb-2">Repuestos sugeridos</p>
          <div className="flex flex-wrap gap-1.5">
            {suggestedParts.map((part) => (
              <span
                key={part.id}
                className="inline-flex items-center text-xs bg-white text-gray-700 px-2 py-1 rounded border border-gray-200"
              >
                {part.name}
      ...
      <div className="flex gap-2 pt-2">
        ...
        <button
          ...
          className="flex-1 bg-white hover:bg-gray-50 active:bg-gray-100 text-gray-700 text-sm font-medium py-2.5 px-4 rounded-md border border-gray-300 transition-colors"
        >
          Ver detalle
        </button>
      </div>
